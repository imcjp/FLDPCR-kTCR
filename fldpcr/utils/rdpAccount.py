...
def epochAllowed(sample_rate, stepsHasRunned, requiredSteps, maxEps, delta: float, sigma=1.0, alphas=None):
    epsUsed, best_alpha = getPrivacySpentWithFixedNoise(sample_rate, stepsHasRunned + requiredSteps, delta, sigma=sigma,
                                                        alphas=alphas);
    if maxEps < epsUsed:
        minStep = 0
        ...
        while minStep < maxStep:
            ...
            epsUsed, best_alpha = getPrivacySpentWithFixedNoise(sample_rate, stepsHasRunned + midStep, delta,
