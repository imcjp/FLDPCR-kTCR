...
class PriClient(object):
    ...
    def setup(self, **client_config):
        ...
        if client_config["dp_config"]['isFixedClientT']:
            ...
            self.sigma = getMinSigma(sample_rate=sample_rate, num_steps=self.maxAllowedIter, delta=self.delta,
                                     requireEps=self.epsilon);
            print(
                f'Under maximum iteration number = {self.maxAllowedIter} and sample_rate = {sample_rate}, the sigma of F{1 + self.id} is {self.sigma}.')
        else:
            self.sigma = float(client_config["dp_config"]["sigma"])
            sample_rate = 1 / len(self.dataloader);
            self.maxAllowedIter = getClientT(sample_rate=sample_rate, maxEps=self.epsilon, delta=self.delta,
                                             sigma=self.sigma)
            print(
                f'Under sigma = {self.sigma} and sample_rate = {sample_rate}, the maximum iteration number of F{1 + self.id} is {self.maxAllowedIter}.')
